...
def transport_labels(labels):
    for i in labels:
        labels[i] = labels[i] if labels[i] == 1 else 0
    return labels
...
model = xgb.train(plst, xgb_train, num_rounds, watchlist, early_stopping_rounds=10000) #, pred_margin=1
# model.save_model('./model/xgb.model') # 用于存储训练出的模型
print("best best_ntree_limit", model.best_ntree_limit)
y_pred = model.predict(xgb_test, ntree_limit=model.best_ntree_limit)
print ('error=%f' % (sum(1 for i in range(len(y_pred)) if -(-1)**int(y_pred[i] > 0) != y_test[i]) / float(len(y_pred))))
# 输出运行时长
